...
Semester project of Miha Pompe with the goal to develop a novel mathematically informed architecture of a PINN to solve the nuclear decay equation.
...
| Size | Stiffness | Relative error | Relative final error |
|---|---|---|---|
|17x17|$10^{30}$|0.79%|0.94%|
|50x50|$10^{16}$|0.0009%|0.0011%|
|100x100|$10^{18}$|0.13%|0.23%|
...
First clone the repository
    ...
    git clone https://gitlab.psi.ch/AMAS-students/miha-semester.git
...
The biggest impact it caused by lowering the tolerance_change and then by slightly increasing max_iter and sometimes learning rate.
...
Burnup matrix of size 43×43 with stiffness |A| = 3, 1 × 1011 was cropped from dataset ENDFB71. The orange line represents the mean of the final error which is 2, 2×10−4. The total relative error is 8,2×10−5. Computation time of CRAM was 15s and of the PINN was 126s (on CPU).
...
- Implement and test architecture to solve the burn-up problem.
    ...
    - The network is able to solve the decay problem (50x50, stiffness 5x10^16, rel error 3.5*10^-6, time 332s) with higher accuracy but longer time.
...
- Self-adjusting weights. Rule: when log10(IC_loss) - log10(ODE_loss) >= 5: weights = [1,1]
...
- Implementing the paper (50x50, t_max =2.5, t_steps = 10000, stiffness = 5x10^16)
...
- Test the model on bigger matrices (without stiffness reduction)
    ...
    |---|---|---|---|---|
    | 5x5 | 5x10^8 | 5x10^-7 | 0.4 s | 12.8 s |
    | 12x12 | 10^24 | 0.9 % | 0.7 s | 13.3 s |
    | 17x17 | 10^30 | 1.8 % | 1.9 s | 13.4 s |
    | 100x100 | 10^18 | 8.1 % | 29.5 s | 19.8 s |
    | 100x100 | 10^10 | 5x10^-6 | 30.3 s | 23.4 s |
    | 500x500 | 8x10^24 | 54 % | 38 min | 3.5 min |
